...
- **一个仓库 = 一个任务**：loopx issue-fix 的模型就是「一个 goal 挂多个
  issue-todo」。入库时若该仓库已有非终态任务，新 Issues **默认作为子任务
  （todo）并入该任务**；composer 目标下拉可显式改选「新建任务」另开一个。
  worker 侧有创建时硬去重（同仓库已有 goal 拒绝新建），并且**按 issue URL
  去重**（重复粘贴同一 issue 会跳过、不写重复 todo）。历史原因产生的同仓库
  多任务（-2/-6 后缀）是旧版行为，应清理合并。
